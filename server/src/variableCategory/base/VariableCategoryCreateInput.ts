/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CategoryCreateNestedManyWithoutVariableCategoriesInput } from "./CategoryCreateNestedManyWithoutVariableCategoriesInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";
import { UserWhereUniqueInput } from "../../user/base/UserWhereUniqueInput";
import { VariableCreateNestedManyWithoutVariableCategoriesInput } from "./VariableCreateNestedManyWithoutVariableCategoriesInput";

@InputType()
class VariableCategoryCreateInput {
  @ApiProperty({
    required: false,
    type: () => CategoryCreateNestedManyWithoutVariableCategoriesInput,
  })
  @ValidateNested()
  @Type(() => CategoryCreateNestedManyWithoutVariableCategoriesInput)
  @IsOptional()
  @Field(() => CategoryCreateNestedManyWithoutVariableCategoriesInput, {
    nullable: true,
  })
  categories?: CategoryCreateNestedManyWithoutVariableCategoriesInput;

  @ApiProperty({
    required: false,
    type: () => UserWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => UserWhereUniqueInput)
  @IsOptional()
  @Field(() => UserWhereUniqueInput, {
    nullable: true,
  })
  user?: UserWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => VariableCreateNestedManyWithoutVariableCategoriesInput,
  })
  @ValidateNested()
  @Type(() => VariableCreateNestedManyWithoutVariableCategoriesInput)
  @IsOptional()
  @Field(() => VariableCreateNestedManyWithoutVariableCategoriesInput, {
    nullable: true,
  })
  variables?: VariableCreateNestedManyWithoutVariableCategoriesInput;
}

export { VariableCategoryCreateInput as VariableCategoryCreateInput };
